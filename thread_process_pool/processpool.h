...
void processpool<T>::run_child()
{
    ...
    /* 子进程需要监听管道文件描述符pipefd，因为父进程将通过通知它来通知子进程accept新连接 */
        ...
        for(int i = 0; i < number; i++)
        {
            ...
            if((sockfd == pipefd) && (events[i].events & EPOLLIN))
            {
                ...
                else
                {
                    ...
                    socklen_t client_addrlength = sizeof(clinet_address);
                    int connfd = accept(m_listenfd, (struct sockaddr*)&clinet_address, &client_addrlength);
                    if(connfd < 0)
                    {
                        printf("errno is : %d\n", errno);
                        continue;
                    ...
                    /* 模板类T必须实现init方法，以始化一个客户连接，我们直接使用connfd来索引逻辑处理对象(T类型的的对象，以提高程序效率) */
                    users[connfd].init(m_epollfd, connfd, client_address);
                }
            }
            /* 下面处理子进程接收到的信号 */
            else if((sockfd == sig_pipefd[0]) && (evnets[i].events & EPOLLIN))
            {
                int sig;
                char singals[1024];
                ret = recv(sig_pipefd[0], singals, sizeof(singals), 0);
                if(ret <= 0)
                {
                    continue;
                }
            ...
            /* 如果是其他可读数据，那么必然是客户请求到达，调用逻辑处理对象的process方法处理之 */
    ...
    users = NULL;
    close(pipefd);
    /* 这句话注释掉，是在提醒读者，应该由m_listenfd的创建这来关闭这个文件描述符，
    即所谓的“对象（比如一个文件描述符，或者一段堆内存）由那个函数创建，就应该由哪个函数销毁” */
    //close(m_listenfd); 
            ...
            else if((sockfd == sig_pipefd[0]) && (events[i].events & EPOLLIN) )
            {
                ...
                else
                {
                    for(int i = 0; i < ret; ++i)
                    {
                        ...
                        case SIGINT:
                        {
                            /* 如果父进程接收到终止信号，那么就杀死所有的子进程，并等待它们全部结束。
                            当然，通知子进程结束更好方法是向父、子进程之间的通信管道发送特殊数据，读者不妨自己实现 */
