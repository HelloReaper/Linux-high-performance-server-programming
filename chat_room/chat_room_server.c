...
int main(int argc,char* argv[])
{
    ...
    /* 创建user数组，分配FD_LIMIT个client_data对象，可以预期：每个可能的socket连接都可以获得这样一个对象，
    并且socket的值可以直接用来索引（作为数组下标）socket连接对应的client_data对象，这是将socket和客户数据关联的简单而高效的方式 */
    ...
    while(1)
    {
        ...
        for(int i=0;i<user_counter+1;++i)
        {
            if((fds[i].fd==listenfd)&&(fds[i].revents&POLLIN))
            {
                ...
                pritnf("comes a new user,now have %d users\n",user_counter);
            }
