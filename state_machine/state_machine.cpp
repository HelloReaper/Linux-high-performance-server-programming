...
/* 服务器处理HTTP请求的结果: NO_REQUEST表示请求不完整，需要继续读取客户数据;GET_REQUEST表示获得了一个完整的客户请求;
    BAD REQUEST表示客户请求有语法错误;FORBIDDEN_REQUEST表示客户对资源没有足够的访问权限；
...
/* 分析请求行 */
enum HTTP_CODE parse_requestline(char* temp,enum CHECK_STATE& checkstats)
{
...
int main(int argc,char* argv[])
{
    ...
    else
    {
        ...
        int checked_index=0;/* 当前以及分析完了多字节的客户数据 */
        int start_line=0;
        /* 设置主状态机的初始状态 */
